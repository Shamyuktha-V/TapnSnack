                     ...
                     int nCmdShow)
{
    ...
    wincl.hbrBackground = CreateSolidBrush (RGB(173, 216, 230));
    ...
    switch (message)                  /* handle the messages */
    {
        ...
        case WM_COMMAND:
            ...
            else if(LOWORD(wParam)==ID_CMD32)
            {
                ...
                try
                {
                    if(Vm.C.checkBalance(amount,bi)==1 && Vm.C.get_balance()!=0)
                    {
                        MessageBox(hwnd,"Collect your balance at cash dispenser","Success",MB_OK|MB_ICONINFORMATION);
                        SetWindowText(Textbox2,"");
                        system("cls");
                        bi.display_bill(dr,ch,cp);
                        cout<<"BALANCE - "<<Vm.C.get_balance();
                        MessageBox(hwnd,"THANKS FOR YOUR PURCHASE!","Thank you",MB_OK|MB_ICONINFORMATION);
                        DestroyWindow(hwnd);
                    }
                    else if(Vm.C.checkBalance(amount,bi)==1 && Vm.C.get_balance()==0)
                    {
                    ...
                    else if(Vm.C.checkBalance(amount,bi)==0)
                    {
                        throw 10;
                    }
                    else if(Vm.C.checkBalance(amount,bi)==2)
                    {
                        throw "negative";
                    }
                }
                ...
                {
                    MessageBox(hwnd,"INSERT CORRECT AMOUNT","Warning",MB_OK|MB_ICONINFORMATION);
                    SetWindowText(Textbox2,"");
                }
            }
